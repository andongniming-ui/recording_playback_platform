...
def _search_json_for_transaction_code(value) -> str | None:
    if isinstance(value, dict):
        for key, item in value.items():
            normalized_key = str(key).replace("-", "_").lower()
            if normalized_key in TRANSACTION_CODE_KEYS:
                text = _normalize_text(item)
                if text:
                    return text
            nested = _search_json_for_transaction_code(item)
            if nested:
                return nested
...
def _search_xml_for_transaction_code(text: str) -> str | None:
    try:
    ...
    for element in root.iter():
        ...
        if tag in TRANSACTION_CODE_KEYS:
...
def infer_transaction_code(*values: str | None) -> str | None:
    for raw in values:
        text = _normalize_text(raw)
        if not text:
            continue
        xml_value = _search_xml_for_transaction_code(text)
        if xml_value:
            return xml_value
        try:
        ...
        if parsed is not None:
            json_value = _search_json_for_transaction_code(parsed)
            if json_value:
                return json_value
    return None
