...
def _fill_governance_fields(payload: dict) -> dict:
    ...
    if isinstance(scene_key, str) and scene_key.startswith("unknown_tx|") and transaction_code:
        scene_key = None
    payload["scene_key"] = scene_key or build_scene_key(
        transaction_code,
...
async def bulk_delete_test_cases(
    ...
    case_ids = sorted({case_id for case_id in body.ids if case_id})
    if not case_ids:
        return BulkDeleteResponse(deleted=0)

    result = await db.execute(select(TestCase.id).where(TestCase.id.in_(case_ids)))
    existing_ids = list(result.scalars().all())
    if not existing_ids:
        return BulkDeleteResponse(deleted=0)

    await db.execute(delete(SuiteCase).where(SuiteCase.test_case_id.in_(existing_ids)))
    await db.execute(update(ReplayResult).where(ReplayResult.test_case_id.in_(existing_ids)).values(test_case_id=None))
    await db.execute(delete(TestCase).where(TestCase.id.in_(existing_ids)))
    await db.commit()
    return BulkDeleteResponse(deleted=len(existing_ids))
...
async def delete_test_case(
    ...
    if not tc:
        raise HTTPException(status_code=404, detail="Test case not found")
    await db.execute(delete(SuiteCase).where(SuiteCase.test_case_id == case_id))
    await db.execute(update(ReplayResult).where(ReplayResult.test_case_id == case_id).values(test_case_id=None))
    await db.delete(tc)
    await db.commit()
