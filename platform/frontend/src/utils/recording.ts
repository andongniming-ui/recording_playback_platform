...
function formatLocalLikeDateTime(value: string): string | null {
  ...
  const local = parsed
    ...
    .replace(/\//g, '-')
  return `${text} (北京时间 ${local})`
}

function annotateDateTimes(value: unknown, keyHint?: string): unknown {
  if (value == null) {
    return value
  }
  if (typeof value === 'string') {
    const shouldFormat =
      !keyHint
      || /(?:^|_)(?:at|time)$|created_at|updated_at|request_time|response_time/i.test(keyHint)
    if (!shouldFormat) {
      return value
    }
...
export function formatRecordingSubCallValue(value: unknown): string {
  ...
  if (typeof normalized === 'string') {
    return annotateDateTimes(normalized) as string
  }
  try {
    return JSON.stringify(annotateDateTimes(normalized), null, 2)
  } catch {
