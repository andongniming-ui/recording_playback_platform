    ...
    <div v-if="app" class="detail-stack">
      ...
      <n-card title="应用概览">
        ...
        <n-descriptions bordered :column="2" label-placement="left">
          ...
          <n-descriptions-item label="交易码提取字段">{{ transactionCodeFieldSummary }}</n-descriptions-item>
          ...
          <n-collapse-item title="交易码映射" name="tx-mappings">
            <n-space vertical :size="12">
              <n-alert type="info" :show-icon="false">
                当前应用按这些字段提取交易码：<code>{{ transactionCodeFieldSummary }}</code>
              </n-alert>
              <n-alert type="info" :show-icon="false">
...
const transactionCodeFieldSummary = computed(() => {
  const fields = app.value?.transaction_code_fields || []
