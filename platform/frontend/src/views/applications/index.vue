<template>
  <n-space vertical :size="16" class="applications-page">
    <n-space justify="space-between" align="center">
      ...
      <n-space>
        <n-button
          v-if="userStore.role === 'admin' && selectedAppIds.length > 0"
          type="error"
          @click="deleteSelectedApps"
        >
          批量删除{{ selectedAppIds.length > 0 ? ` (${selectedAppIds.length})` : '' }}
        </n-button>
  ...
  >
    <n-form ref="formRef" :model="form" label-placement="left" label-width="120px">
      ...
      <n-form-item label="交易码提取字段">
        <n-space vertical style="width: 100%">
          <n-alert type="info" :show-icon="false">
            按应用维度配置交易码提取字段，录制时优先按这里的字段名从请求/响应报文里提取交易码。
            推荐按优先级一行一个，例如 <code>txnCode</code>、<code>code</code>、<code>sys_code</code>。
          </n-alert>
          <n-input
            v-model:value="form.transaction_code_fields"
            type="textarea"
...
type AppRow = {
  ...
  transaction_code_fields?: string[] | null
...
type AppForm = {
  ...
  sample_rate: number
  transaction_code_fields: string
...
const columns = computed<DataTableColumns<AppRow>>(() => [
  ...(userStore.role === 'admin' ? [{ type: 'selection' as const }] : []),
  {
    title: '名称',
    key: 'name',
    width: 220,
  ...
  },
  {
    title: '操作',
    key: 'actions',
    width: userStore.role === 'admin' ? 440 : canEdit ? 380 : 140,
    render: (row) => {
      const normalizedStatus = normalizeAgentStatus(row.agent_status)
...
function openEdit(app: Record<string, any>) {
  ...
  form.value = {
    ...
    transaction_code_fields: Array.isArray(app.transaction_code_fields)
      ? app.transaction_code_fields.join('\n')
      : '',
...
async function deleteSelectedApps() {
  ...
  try {
    const res = await applicationApi.bulkDelete({ ids: selectedAppIds.value.map(Number) })
    message.success(`已删除 ${res.data.deleted} 个应用`)
    await loadApps()
  } catch (error: any) {
