...
const caseLookup = computed<Record<number, any>>(() => {
  const lookup: Record<number, any> = { ...caseMap.value }
...
const selectedSceneKeyOwners = computed<Record<string, number>>(() => {
  const owners: Record<string, number> = {}
  for (const rawId of selectedCaseIds.value) {
    const caseId = Number(rawId)
    const sceneKey = caseLookup.value[caseId]?.scene_key
    if (!sceneKey || owners[sceneKey] != null) {
      continue
    }
    owners[sceneKey] = caseId
  }
  return owners
})
...
function getSceneKeyConflict(caseId: number) {
  const sceneKey = caseLookup.value[caseId]?.scene_key
  if (!sceneKey) {
    return null
  }
  const ownerId = selectedSceneKeyOwners.value[sceneKey]
  if (ownerId == null || ownerId === caseId) {
    return null
  }
  return {
    sceneKey,
    ownerId,
    ownerName: caseLookup.value[ownerId]?.name || `用例 #${ownerId}`,
  }
}

function isCaseSelectionDisabled(row: any) {
  return getSceneKeyConflict(Number(row.id)) != null
}

function getSceneKeyConflictText(row: any) {
  const conflict = getSceneKeyConflict(Number(row.id))
  if (!conflict) {
    return ''
  }
  return `与已选用例“${conflict.ownerName}”场景重复`
}

function collectDuplicateSceneKeyConflicts(caseIds: Array<string | number>) {
  const sceneKeyOwners: Record<string, { caseId: number; name: string }> = {}
  const conflicts: Array<{ sceneKey: string; currentName: string; ownerName: string }> = []
  for (const rawId of caseIds) {
    const caseId = Number(rawId)
    const testCase = caseLookup.value[caseId]
    const sceneKey = testCase?.scene_key
    if (!sceneKey) {
      continue
    }
    const currentName = testCase?.name || `用例 #${caseId}`
    const owner = sceneKeyOwners[sceneKey]
    if (!owner) {
      sceneKeyOwners[sceneKey] = { caseId, name: currentName }
      continue
    }
    if (owner.caseId !== caseId) {
      conflicts.push({ sceneKey, currentName, ownerName: owner.name })
    }
  }
  return conflicts
}

const addCaseCols = computed(() => [
  { type: 'selection' as const, disabled: (row: any) => isCaseSelectionDisabled(row) },
  {
    title: '名称',
    key: 'name',
    ellipsis: { tooltip: true },
    render: (row: any) => {
      const conflictText = getSceneKeyConflictText(row)
      if (!conflictText) {
        return row.name
      }
      return h('div', { style: 'display:flex;flex-direction:column;gap:2px;' }, [
        h('span', row.name),
        h('span', { style: 'color:#d03050;font-size:12px;' }, conflictText),
      ])
    },
  },
  {
...
async function addCases() {
  ...
  const duplicateConflicts = collectDuplicateSceneKeyConflicts(selectedCaseIds.value)
  if (duplicateConflicts.length > 0) {
    const conflict = duplicateConflicts[0]
    message.error(`存在重复场景样本：${conflict.ownerName} 与 ${conflict.currentName} 不能同时加入同一套件`)
    return
  }
  ...
  try {
    ...
    if (typeof detail === 'string' && detail.startsWith('Duplicate scene_key in suite selection:')) {
      message.error(`存在重复场景样本，不能加入同一套件：${detail.slice(detail.indexOf(':') + 1).trim()}`)
    } else {
      message.error(detail || '更新套件失败')
    }
  } finally {
