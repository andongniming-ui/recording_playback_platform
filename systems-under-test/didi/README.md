...
```sql
source /home/recording_playback_platform/systems-under-test/didi/create_databases.sql;
```
...
如果你使用的是仓库自带的 MySQL 容器，请记得把端口改成 `3307`：

```bash
export DIDI_DB_PORT=3307
```

`start.sh` 现在会在启动前校验 AREX Agent JAR 是否存在且可读。默认路径是 `/home/test/arex-agent/arex-agent.jar`，也可以通过下面任一环境变量覆盖：

- `AREX_AGENT_JAR_PATH`
- `AR_AREX_AGENT_JAR_PATH`

例如：

```bash
export AREX_AGENT_JAR_PATH=/opt/arex/arex-agent.jar
```
...
```bash
cd /home/recording_playback_platform/systems-under-test/didi/didi-system-a
chmod +x start.sh
DIDI_DB_PORT=3307 DIDI_SQL_INIT_MODE=never ./start.sh
```
...
```bash
cd /home/recording_playback_platform/systems-under-test/didi/didi-system-b
chmod +x start.sh
DIDI_DB_PORT=3307 DIDI_SQL_INIT_MODE=never ./start.sh
```
...
这两个系统和当前 `/home/recording_playback_platform` 目录结构是匹配的，原因是：
...
```bash
cd /home/recording_playback_platform/systems-under-test/didi
