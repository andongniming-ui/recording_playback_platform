...
validate_arex_agent() {
  if [ ! -f "$AREX_AGENT_JAR_PATH" ]; then
    ...
    echo "Set AREX_AGENT_JAR_PATH or AR_AREX_AGENT_JAR_PATH to a valid arex-agent.jar before starting didi-system-a." >&2
    exit 1
  fi

  if [ ! -s "$AREX_AGENT_JAR_PATH" ]; then
    echo "ERROR: AREX agent jar is empty: $AREX_AGENT_JAR_PATH" >&2
    echo "Restore the jar or point AREX_AGENT_JAR_PATH to a valid copy before starting didi-system-a." >&2
    exit 1
  fi
