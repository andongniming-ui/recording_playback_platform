...
public class StoreMapper {
    ...
    // COMPARE: descending order methods used by compare system
    public List<RentalStore> findAllOrderByCreateTimeDesc() {
        return jdbc.query("SELECT * FROM rental_store ORDER BY create_time DESC", rowMapper);
    }

    public List<RentalStore> findByStatusOrderByCreateTimeDesc(Integer status) {
        return jdbc.query("SELECT * FROM rental_store WHERE status=? ORDER BY create_time DESC", rowMapper, status);
    }
}
