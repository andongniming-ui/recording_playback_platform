...
import com.rental.compare.mapper.*;
...
public class StatisticsService {
    ...
    public List<StatisticsBody.DailyStatRes> daily(StatisticsBody.DailyReq req) {
        ...
        // Group by store first (COMPARE: store-first grouping; BASE: date-first grouping)
        Map<Long, StatisticsBody.DailyStatRes> statMap = new LinkedHashMap<>();
        for (RentalOrder o : orders) {
            Long storeKey = o.getStoreId();
            StatisticsBody.DailyStatRes stat = statMap.get(storeKey);
            if (stat == null) {
                stat = new StatisticsBody.DailyStatRes();
                stat.setStatDate(DateTimeUtil.format(o.getCreateTime()).substring(0, 10));
            ...
            stat.setCreateTime(DateTimeUtil.format(o.getCreateTime()).substring(0, 10));
        }
    ...
    public StatisticsBody.UserReportRes userReport(StatisticsBody.UserReportReq req) {
        ...
        // COMPARE: includes membership_level field (not in BASE)
