...
import com.rental.compare.mapper.StoreMapper;
...
public class StoreService {
    ...
    public StoreBody.StoreListRes list(StoreBody.ListReq req) {
        ...
        if (req.getStatus() != null) {
            stores = storeMapper.findByStatusOrderByCreateTimeDesc(req.getStatus());
        } else {
            stores = storeMapper.findAllOrderByCreateTimeDesc();
        }
        long elapsed = System.currentTimeMillis() - start;
        TransactionLogUtil.addDbCall("SELECT rental_store list ORDER BY create_time DESC elapsed=" + elapsed + "ms");
